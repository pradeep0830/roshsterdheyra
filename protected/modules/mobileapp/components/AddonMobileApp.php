...
class AddonMobileApp
{
	...
	{
		$error_string='';
		if (is_array($error) && count( (array) $error)>=1){
	...
	public static function computeCart($data='')
	{
		...
		if (is_array($cart) && count( (array) $cart)>=1){
			foreach ($cart as $val) {
		    	...
		    	/*2 flavor*/
		    	if ($food['two_flavors']==2){
		    		if(is_array($addon_prices) && count( (array) $addon_prices)>=1){
		    	...
		    	$cart_content[]=array(
		    	  ...
		    	  'category_id'=>isset($val['category_id'])?$val['category_id']:'' //cart category
		    	);
	       ...
	       if ( $data['transaction_type']=="delivery"){	
	    	...
	    	if (method_exists("FunctionsV3","getMerchantOffersActive")){    	
		    	if ( $offer=FunctionsV3::getMerchantOffersActive($mtid,$data['transaction_type'])){
		    		...
		    		if(isset($offer['applicable_to'])){
		        	  if (!empty($offer['applicable_to'])){
		        	  	  if ($offer['applicable_to']!="all"){
		        	  	     ...
		        	  	     if(!in_array($data['transaction_type'],(array)$applicable_to)){
           ...
           /*get the tax*/
			...
			$grand_total=$subtotal+$delivery_charges+$merchant_packaging_charge+$tax+$tips_amount+$card_fee;
	...
	public static function cartMobile2WebFormat($data='',$post_data='')
	{
		...
		if (self::isArray($data['cart']['cart'])){
			foreach ($data['cart']['cart'] as $val) {				
				$sub_item=array(); $addon_qty=array(); $addon_ids=array();
	...
	public static function getOperationalHours($merchant_id='')
	{
		...
		$open_ends='';
		$open_text='';		
		if (is_array($stores_open_day) && count( (array) $stores_open_day)>=1){
   ...
   public static function getMerchantInfo($merchant_id='')
    ...
    public static function sendOrderEmail($data='',$info='',$order_id='',$full_data='',$ok_send_notification=true)
    {
        ...
        if ( $enabled_alert_notification==""){ 
	    	...
	    	if (is_array($merchant_notify_email) && count( (array) $merchant_notify_email)>=1){
    ...
    public static function getMerchantOffers($merchant_id='')
    {
    	...
    	if (method_exists("FunctionsV3","getOffersByMerchantNew")){
	    	if($res=FunctionsV3::getOffersByMerchantNew($merchant_id)){
	    	   foreach ($res as $val) {
	    	   	   $offer[] = $val;
	    	   }
	    	}
	...
	public static function getAppLanguage()
	{
		...
		if(is_array($enabled_lang) && count( (array) $enabled_lang)>=1){			
    	    	...
    	    	foreach ($res as $val) {
    	    		if(in_array($val,$enabled_lang)){
    	    			...
    	    			if (file_exists($lang_path)){
		    				...
		    				if(is_array($temp_lang) && count( (array) $temp_lang)>=1){
    ...
    public static function displayCashAvailable($merchant_id='', $services='')
    {
		...
		if(is_array($payment_accepted) && count( (array) $payment_accepted)>=1){
	...
	public static function registeredDevice($client_id='',$device_id='',$device_platform='',$app_version="2.5")
	{
		$DbExt=new DbExt; 
		if(!empty($client_id)){
			$params=array(
   ...
   public static function getPaymentMethodNew($mtid='', $services='')
   {   	  
   	  $list=array();
   ...
   public static function getCityList($s='')
   {
   	    ...
   	    if(!empty($s)){
   	    	//$and.=" AND name LIKE '%".$s."%' ";
   	    	$and.=" AND name LIKE ".FunctionsV3::q("%$s%")." ";
   	    }
   ...
   public static function saveGoogleAvatar($link='',$user_id='')
	...
	public static function getCategoryCount($merchant_id='', $days='')
	{
		$and='';
	...
	public static function getAvatar2($client_id='')
    {
    	if ( $res= Yii::app()->functions->getClientInfo($client_id) ){
    		$file=$res['avatar'];
    	} else $file='avatar.jpg';
    ...
    public static function getDishPics($list='')
    {
		...
		if(is_array($dish) && count( (array) $dish)>=1){
			foreach ($dish as $dish_id) {
				if($dish_info = Yii::app()->functions->GetDish($dish_id)){								
					if ($icon_link = AddonMobileApp::getGallerPicURL($dish_info['photo'])){
						$dish_list[]=$icon_link;
					}							
				}						
			}
    ...
    public static function getDriverAvatar($image='')
	{	
		$url='';			
		...
		if (!empty($image)){			
			if (file_exists($path_to_upload."/$image")){														
				$url = Yii::app()->getBaseUrl(true)."/upload/driver/$image";
			}
		}
		
		if(empty($url)){
			$url = Yii::app()->getBaseUrl(true)."/protected/modules/mobileapp/assets/images/user.png";
		}
		return $url;
	}	
    ...
    public static function getAddressBookLocation($id='')
    {
    	$db = new DbExt();
    	$stmt="SELECT * FROM
    ...
    public static function getPushLogsByClientID($client_id='')
    {
    	$db = new DbExt();
    	$stmt="SELECT 
    	id,client_id,device_platform,device_id,push_title,push_message,push_type,status,date_created
    	 FROM
    	{{mobile_push_logs}}
    	WHERE
    	client_id=".FunctionsV3::q($client_id)."
    ...
    public static function getPushLogsByDeviceID($device_id='')
    {
    	$db = new DbExt();
    	$stmt="SELECT
    	id,client_id,device_platform,device_id,push_title,push_message,push_type,status,date_created
    ...
    public static function getNextDeliveryTime($merchant_id='',$day='', $delivery_date='')
	{
		if(!empty($day)){
			$day = strtolower($day);			
			if($res=Yii::app()->functions->getBusinnesHours($merchant_id)){			
				if(array_key_exists($day,(array)$res)){
					$time = explode("-",$res[$day]);
					if(is_array($time) && count( (array) $time)>=1){
					   return array(
					     'time'=>trim(rtrim($time[0])),
					     'date'=>date("Y-m-d",strtotime($delivery_date))
					   );
					}
				} else {
					//echo "not found $day $delivery_date";
					$next_day = date("D", strtotime($delivery_date." +1 day"));
					$next_date = date("Y-m-d", strtotime($delivery_date." +1 day"));
					/*dump("next_day $next_day");			
					dump("next_date $next_date");	*/
					return self::getNextDeliveryTime($merchant_id,$next_day,$next_date);
				}
			}
		}		
		return false;
	...
	public static function getSMScodeByToken($token='')
	{
		$db=new DbExt();
		$stmt="
		SELECT * FROM
	...
	public static function validateOrderSMScode($code='', $token='')
	{
		$db=new DbExt();
		$stmt="
		SELECT * FROM
	...
	public static function isOrderSmsEnabled($merchant_id='')
	{	   
