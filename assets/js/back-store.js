...
jQuery(document).ready(function() {
    ...
    $( document ).on( "click", ".checkout", function() {
        ...
        //validation contact number and payment method
        var contact_number=$("#contact_phone").val();
        ...
        params+= addValidationRequest();
        //window.location.href=sites_url+"/checkout/"+params;
        busy(true);
        $.ajax({
            type: "POST",
            url: ajax_url,
            data: "action=placeOrderMerchant&currentController=merchant&tbl=placeOrderMerchant&"+params,
            dataType: 'json',
            success: function(data){
                busy(false);
                ...
                if (data.code==1) {
                   // window.location.href=data.details.payment_link;
                    window.open(data.details.payment_link, '_blank');
                    console.log(data.details.payment_link);
    ...
    $('#delivery_asap').on('ifChecked', function(event){
        $("#delivery_time").val('');
        $(".recur-order").css("display","none");
    });

    $('#delivery_asap').on('ifUnchecked', function(event){
        $(".recur-order").css("display","block");
    });
...
{
    var params="action=loadItemCart&currentController=merchant&merchant_id="+$("#merchant_id").val();
...
function single_food_item_add(item_id,price,size,category_id, size_id , discount)
{
    ...
    params+="&csrf_token="+$("#YII_CSRF_TOKEN").val();
    ...
    $.ajax({
        type: "POST",
        url: ajax_url,
        data: "action=addToCart&currentController=merchant&"+params,
        dataType: 'json',
        success: function(data){
            busy(false);
            //location.reload();
...
/*shifted oder to next last day*/
/*function for recurring order shifting*/

function shift_my_Order(order_id,r_id)
{
    var params="action=addToOrder&currentController=store&shift_my_order="+r_id+"&&order_id="+order_id;
    params+= addValidationRequest();
    console.log(params);
    busy(true);
    $.ajax({
        ...
        success: function(data){
            ...
            if (data.code==1){
                //window.location.replace(sites_url+"/menu/merchant/"+data.details.restaurant_slug);
                window.location.replace(sites_url+"/receipt?id="+data.details.id+"&date="+data.details.new_date);
                //console.log(data.details);
            } else {
                uk_msg(data.msg);
            }
...
$( document ).on( "click", ".shift-my-order", function() {
    var id=$(this).data("id");
    var r_id=$(this).data("recurring");
    var a=confirm("Are you want to shift your order !!");
    if (a){
        shift_my_Order(id,r_id);
    }
});
