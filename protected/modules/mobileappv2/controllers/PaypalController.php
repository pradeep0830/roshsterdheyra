	...
	{				
		...
		if(empty($error)){
			if ($credentials=PaypalWrapper::getCredentials($merchant_id)){			
				$success_url = websiteUrl()."/".APP_FOLDER."/paypal/success/?reference_id=".urlencode($reference_id)."&trans_type=$trans_type";
		...
		if(!empty($error)){			
			$this->redirect(Yii::app()->createUrl('/'.APP_FOLDER.'/paypal/error',array(
			   'error'=>$error
			 ))); 
		}
	}
