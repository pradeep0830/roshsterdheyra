...
class ApiController extends CController
{
	...
	{		
		...
		// echo "Request api_key::".$_POST['api_key'];
        /*CHECK API HASH KEY*/
		$api_key = isset($_REQUEST['api_key'])?trim($_REQUEST['api_key']):'';   
		// echo "api_key::".$api_key; exit;  
	...
	public function actionaddToCart()
	{
		...
		if ( $res = mobileWrapper::getCart($device_uiid)){			
			 ...
			 $row = isset($data['row'])?$data['row']:'';
				...
				$responce=$DbExt->updateData("{{mobile2_cart}}",array(
			...
			$cart_count=1;
			$responce=$DbExt->insertData("{{mobile2_cart}}",array(
			 'merchant_id'=>$merchant_id,
		     'device_uiid'=>$device_uiid,		     
	...
	public function actionpayNow()
	{
    	...
    	if($res=mobileWrapper::getCart($this->device_uiid)){
		    ...
		    if ($code==1){
				...
				/**
				 * start recurring order for mobile app last step
				 * @meal_subscription bolean
				 * @recurring_start_date data
				 * @recurring_end_date date
				 */
				if(!empty($this->data['recurring_orders']) && isset($this->data['recurring_end_date'])){
					$recurring_order=$this->data['recurring_orders'];
					$recurring_start_date=$this->data['recurring_start_date'];
					$recurring_end_date=$this->data['recurring_end_date'];

					//count the day
					$date1 = date_create($recurring_start_date);
					$date2 = date_create($recurring_end_date);
					$diff = date_diff($date1,$date2);
					$total_day=$diff->format("%a");

					//calculate total price
					$total_recurring_price=$raw['total']['subtotal']*$total_day;
				 ...
				 * @recurring_end_date date
				 */
				if(!empty($this->data['recurring_orders']) && isset($this->data['recurring_end_date']) && $this->data['recurring_orders']=="yes"){
					$recurring_order=$this->data['recurring_orders'];
					$recurring_start_date=$this->data['recurring_start_date'];
					$recurring_end_date=$this->data['recurring_end_date'];
					...
					$recuring_params=array(
						...
						'total_price'=>$total_recurring_price
					);
					//get the dates
					$format='Y-m-d';
					$date_array = array();
					$interval = new DateInterval('P1D');
					$realEnd = new DateTime($recurring_end_date);
					$realEnd->add($interval);
					$period = new DatePeriod(new DateTime($recurring_start_date), $interval, $realEnd);
					foreach($period as $datekey => $datevalue) { 
						$date_array[] = $datevalue->format($format);
					}	
					if($db->insertData("{{order_recurring}}",$recuring_params)){
						$recurring_order_id=Yii::app()->db->getLastInsertID();
						if(!is_numeric($raw['total']['tax'])){
							$raw['total']['tax']=0;
						}
						if(!is_numeric($this->data['cc_id'])){
							$this->data['cc_id']=0;
						}
						if(!is_numeric($this->data['order_change'])){
							$this->data['order_change']=0;
						}
						if(!is_numeric($raw['total']['merchant_packaging_charge'])){
							$raw['total']['merchant_packaging_charge']=0;
						}
						//$params['recurring_id']=$recurring_order_id;								
						/*experiment*/
						$sql = "INSERT INTO mt_order (
							merchant_id,
							client_id,
							recurring_id,
							json_details,
						...
						for($i=0;$i< count($date_array);$i++){
								...
								'".rand(1000,9999)."',
								'".$transaction_type."',
								'".$this->data['payment_provider']."',
								'".$raw['total']['subtotal']."',
								'".$raw['total']['tax']."',
								'".$raw['total']['taxable_total']."',
								'".$raw['total']['total']."',
								'".$raw['total']['delivery_charges']."',
								'".$date_array[$i]."',
								'".$delivery_time."',
								'".$this->data['delivery_asap']."',
								'".FunctionsV3::dateNow()."',
								'".$_SERVER['REMOTE_ADDR']."',
								'".$res['delivery_instruction']."',
								'".$this->data['cc_id']."',
								'".$this->data['order_change']."',
								'null',
								'".$card_fee."',
								'".$raw['total']['merchant_packaging_charge']."',
								'".$donot_apply_tax_delivery."',
								'".FunctionsV3::generateOrderToken()."',
								'mobileapp2',
								'".$is_apply_tax."'
								)";
				...
				/**End Recurring ordering */
				//conditional insert data
				if(!empty($this->data['recurring_orders']) && isset($this->data['recurring_end_date'])){
					//dump($sql);exit();
					$DbExt=new DbExt;
					...
					$sbn_query=$db->insertData("{{order}}",$params);
				}

				//insert order details to databased
				if($sbn_query){
						...
						$sql_order_details ="INSERT INTO `mt_order_details` (`order_id`, `client_id`, `item_id`, `item_name`, `order_notes`, `normal_price`, `discounted_price`, `size`, `qty`, `cooking_ref`, `addon`, `ingredients`, `non_taxable`) VALUES";
									foreach ($raw['item'] as $val) {
										for($i=0;$i< count($res_of_recu_order_id);$i++){
											$sql_order_details .= "('".$res_of_recu_order_id[$i]['order_id']."','".$client_id."','".$val['item_id']."','".$val['item_name']."','".$val['order_notes']."','".$val['normal_price']."','".$val['discounted_price']."','".$val['size_words']."','".$val['qty']."','".$val['sub_item']."','".$val['cooking_ref']."','".$val['ingredients']."','".$val['non_taxable']."')";
				    ...
				    $this->msg = Yii::t("mobile2","Your order has been placed. Reference # [order_id]",array(
				      '[order_id]'=>$order_id." Order Pin <b>".$order_otp."</b>"
				    ));
