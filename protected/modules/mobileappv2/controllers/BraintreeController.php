	...
	public function actionIndex()
	{
		...
		if(empty($error)){
			...
			if($client_token=BraintreeClass::generateCLientToken($merchant_type,$client_id,$merchant_id)){
				...
				$success_url = websiteUrl()."/".APP_FOLDER."/braintree/success/?reference_id=".urlencode($reference_id);
		...
		if(!empty($error)){				
			$this->redirect(Yii::app()->createUrl('/'.APP_FOLDER.'/braintree/error',array(
			   'error'=>$error
			))); 
		}
	}
