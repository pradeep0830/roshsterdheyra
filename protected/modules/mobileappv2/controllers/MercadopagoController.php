...
class MercadopagoController extends CController
{
	...
	public function actionIndex()
	{
		...
		if(empty($error)){
			if ($credentials = mercadopagoWrapper::getCredentials($merchant_id)){
				...
				$success_url = websiteUrl()."/".APP_FOLDER."/mercadopago/success?reference_id=".urlencode($reference_id)."&trans_type=$trans_type";
		...
		if(!empty($error)){						
			$this->redirect(Yii::app()->createUrl('/'.APP_FOLDER.'/mercadopago/error',array(
			   'error'=>$error
			))); 
		}
	}
	...
	public function actionsuccess()
	{
		...
		if(!empty($error)){						
			$this->redirect(Yii::app()->createUrl('/'.APP_FOLDER.'/stripe/error',array(
			   'error'=>$error
			))); 
		} 
	}
