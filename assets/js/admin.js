...
/**
 * validate regex for hex color code
 */
// regular function
...
{		    
    ...
    epp_table = $('#table_list').dataTable({
		   ...
		   "fnRowCallback": function(nRow, aaData, iDisplayIndex, iDisplayIndexFull) {
			 if (aaData[9]) {
				 if(isHexColor(aaData[9])){
					//$('td', nRow).css('color', aaData[14]);
					$(nRow).children("td").first().css('background-color', aaData[9]);
					$(nRow).children("td").first().css({"color":"#FCFCFC","font-weight":"600"});
				 }
...
/*start@sobhon for button*/
	function display_advance_filter(){
	 var params=$("#advanceFilter").serialize();
	    params+="&currentController=merchant";
		params+=addValidationRequest();
	 	console.debug(params);
	 	console.log(ajax_url+"?"+params);
		epp_table.fnReloadAjax(ajax_url+"?"+params);
	}
/*end@sobhon for button*/
