...
class StoreController extends CController
{
	...
	{	
		/*start@recurring*/
		if(isset($_SESSION['kr_recurring_order']['start_date'])){
	...
	public function actionReceipt()
	{
		...
		if ($data=Yii::app()->functions->getOrder2($_GET['id'])){	
			/*start@sbn*/
			$order_top=rand(1000,9999);
			$id=$_GET['id'];		 	
			$params = array('otp'=>$order_top);
			$DbExt->updateData("{{order}}",$params,'order_id',$id);
			$sbn=Yii::app()->functions->getOrder2($id);
			/*dump($sbn); die();*/
			/*end@sbn*/
	...
	{		
		...
		//$this->render('payuinit-merchant-success');
	}
	
	public function actionPayuSuccess(){
	$this->render('payuinit-merchant-success');
	}
	
	public function actionPayufail()
	{		
		$this->render('payuinit-merchant-fail');
	}
	...
	{	
		$db=new DbExt();	
		//dump($_SESSION['confirm_order_data']);die();  		
