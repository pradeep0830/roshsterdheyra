    ...
    if (is_numeric($mt_delivery_miles)){    	
	    ...
	    public function setDeliveryOptions()
	    {
		   ...
		   if(!empty($this->data['delivery_date'])){
               ...
               /**check if the customer enable recurring_date */
		   if(isset($this->data['recurring_date_start']) && isset($this->data['recurring_date_end'])){			
			//calculate the time
			$start_r = strtotime($this->data['recurring_date_start']); 
			$end_r = strtotime($this->data['recurring_date_end']);
			$datediff = $end_r-$start_r;			 
			$total_recurring_day= round($datediff / (60 * 60 * 24));
			//get the total price
			 $sbn_total=$_SESSION['kmrs_subtotal'];
			 $update_price=$sbn_total*$total_recurring_day;
			 $_SESSION['kmrs_subtotal']=$update_price;
		   
			 $_SESSION['kr_recurring_order']['start_date']=$this->data['recurring_date_start'];
			 $_SESSION['kr_recurring_order']['end_date']=$this->data['recurring_date_end'];
			 $_SESSION['kr_recurring_order']['total_day']=$total_recurring_day;
			 $_SESSION['kr_recurring_order']['total_total_price']=$update_price;
		  }
		  /**end@ check if the customer enable reucrring_date */
		...
		/*end@recurring price set*/
		    ...
		    /*end recurring order*/		    			
	    			...
	    			if (is_array($raw) && count($raw)>=1){	    				
					  ...
					  'otp'=>isset($_SESSION['confirm_order_data']['otp'])?$_SESSION['confirm_order_data']['otp']:'',
...
//dump($params); die();
	    		        ...
	    		        dump($params);
	    		        die();*/
	    		        /*start inserting data recuring order
							$recuring_params=array(
								'create_date'=>date('Y-m-d'),
								'client_id'=>Yii::app()->functions->getClientId(),
								'merchant_id'=>$this->data['merchant_id'],
								'start_date'=>$_SESSION['kr_recurring_order']['start_date'],
								'end_date'=>$_SESSION['kr_recurring_order']['end_date']
							);
							if($this->insertData("{{order_recurring}}",$recuring_params)){
								$recurring_order_id=Yii::app()->db->getLastInsertID();
								$params['recurring_id']=$recurring_order_id;
							}	
						
				end inserting data recuring order*/
/*start inserting data recuring order*/
							if($_SESSION['kr_recurring_order']['start_date'] || $_SESSION['kr_recurring_order']['end_date']){
								$recuring_params=array(
									'create_date'=>date('Y-m-d'),
									'client_id'=>Yii::app()->functions->getClientId(),
									'merchant_id'=>$this->data['merchant_id'],
								...
								$realEnd = new DateTime($_SESSION['kr_recurring_order']['end_date']);
								$realEnd->add($interval);
								$period = new DatePeriod(new DateTime($_SESSION['kr_recurring_order']['start_date']), $interval, $realEnd);
								foreach($period as $datekey => $datevalue) { 
									$date_array[] = $datevalue->format($format);
								}					
								...
								if($this->insertData("{{order_recurring}}",$recuring_params)){
									...
									$params['recurring_id']=$recurring_order_id;								
									/*experiment*/
									$sql = "INSERT INTO mt_order (merchant_id,client_id,recurring_id,json_details,otp,trans_type,payment_type,sub_total,tax,taxable_total,total_w_tax,delivery_charge,delivery_date,delivery_time,delivery_asap,date_created,ip_address,delivery_instruction,cc_id,order_change,payment_provider_name,apply_food_tax,calculation_method) VALUES";
									for($i=0;$i< count($date_array);$i++){
											$sql .= "('".$this->data['merchant_id']."','".$params['client_id']."','".$params['recurring_id']."','".$params['json_details']."','".rand(1000,9999)."','".$params['trans_type']."','".$params['payment_type']."','".$single_price."','".$params['tax']."','".$params['taxable_total']."','".$params['total_w_tax']."','".$params['delivery_charge']."','".$date_array[$i]."','".$params['delivery_time']."','".$params['delivery_asap']."','".$params['date_created']."','".$params['ip_address']."','".$params['delivery_instruction']."','".$params['cc_id']."','".$params['order_change']."','".$params['payment_provider_name']."','".$params['apply_food_tax']."','".$params['calculation_method']."')";
								...
								//dump($sql);die();
								$sbn_query=Yii::app()->db->createCommand($sql)->execute();
							}
						/*end inserting data recuring order*/
					if($_SESSION['kr_recurring_order']['start_date'] || $_SESSION['kr_recurring_order']['end_date']){
							$DbExt=new DbExt;
							$check_insetdata=$sbn_query;
							$stmt="SELECT order_id FROM `mt_order` order by order_id desc limit ".$sbn_query;
							$res_of_recu_order_id=$DbExt->rst($stmt);
						}else{
							$check_insetdata= $this->insertData("{{order}}",$params);
						}
	    				...
	    				if ($check_insetdata){
		    			   ...
		    			   /*VOUCHER*/
                            if ($has_voucher==TRUE){
				            	...
				            	if(!empty($res_of_recu_order_id))
								{
									/*insert order data for recurring order*/
									$sql_order_details ="INSERT INTO `mt_order_details` (`order_id`, `client_id`, `item_id`, `item_name`, `order_notes`, `normal_price`, `discounted_price`, `size`, `qty`, `cooking_ref`, `addon`, `ingredients`, `non_taxable`) VALUES";
									foreach ($raw['item'] as $val) {
										for($i=0;$i< count($res_of_recu_order_id);$i++){
											$sql_order_details .= "('".$res_of_recu_order_id[$i]['order_id']."','".Yii::app()->functions->getClientId()."','".$val['item_id']."','".$val['item_name']."','".$val['order_notes']."','".$val['normal_price']."','".$val['discounted_price']."','".$val['size_words']."','".$val['qty']."','".$val['sub_item']."','".$val['cooking_ref']."','".$val['ingredients']."','".$val['non_taxable']."')";
												if($i < (count($res_of_recu_order_id) -1 )){
													$sql_order_details .=",";
												}
										}
									}
									//dump($sql_order_details); die();
									$sbn_query_o_d=Yii::app()->db->createCommand($sql_order_details)->execute();
									/*end insert order data for recurring order*/
								}else{
									foreach ($raw['item'] as $val) {		    					
		    				...
		    				if(empty($order_id)){
								$new_array=array();
								foreach($res_of_recu_order_id as $key => $value){	
									$new_array[]=$value;
								}
								$order_id=$new_array[0]['order_id']."&".http_build_query($new_array);
							}
	    ...
	    {	    	
	    	//recurring date shifting
			if(isset($this->data['order_id']) && isset($this->data['shift_my_order'])){
				if ( $res=Yii::app()->functions->getOrder($this->data['order_id'])){	
					if($res_recur=Yii::app()->functions->getRecurringData($res['recurring_id'])){
						$recurring_end_date=$res_recur['end_date'];
						$date=new DateTime($recurring_end_date);
						$new_date=$date->modify('+1 day');
						//update the order to table
						$params=array('delivery_date'=>$new_date->format("Y-m-d"));
						$params1=array('end_date'=>$new_date->format("Y-m-d"));
						$update_date=$this->updateData("{{order}}",$params,'order_id',$this->data['order_id']);
						$update_recurring=$this->updateData("{{order_recurring}}",$params1,'id',$res['recurring_id']);
						//dump($update_date);die();
						if($update_date && $update_recurring){
							$this->code=1;
							$this->msg="ok";
							$this->details=array(
						...
						}else $this->msg=t("Order details not updated");			
						
					}else $this->msg=t("Order not available");
				 }
			}//end recurring date shifting
         ...
         * on demand siva teja redy garu
         * "Deyyam kantey Bhayyam maa cheddadandi".
         */
        public function advanceFilterOption(){
            $and='';
            if (!empty($this->data['status'])){
                if (!empty($this->data['status'])){
                    $and=" AND status ='".$this->data['status']."'";
                }
            }
            if(!empty($this->data['trans_type'])){
                $and.=" AND trans_type='".$this->data['trans_type']."'";
            }
            if(!empty($this->data['method'])){
                $and.=" AND payment_type='".$this->data['method']."'";
            }
            if(!empty($this->data['platform'])){
                $and.=" AND request_from='".$this->data['platform']."'";
            }
            //dump($and);
            ...
            if ( $res=$DbExt->rst($stmt)){
                ...
                foreach ($res as $val) {
                    $new='';
                    $action="<div class=\"uk-button-group\">";
                    $action.="<a data-id=\"".$val['order_id']."\" class=\"edit-order uk-button uk-button-default  uk-button-small\" href=\"javascript:\">".Yii::t("default","Edit")."</a>";
                    $action.="<a data-id=\"".$val['order_id']."\" class=\"view-receipt uk-button uk-button-default  uk-button-small\" href=\"javascript:\">".Yii::t("default","View")."</a>";

                    $action.="<a data-id=\"".$val['order_id']."\" class=\"view-order-history uk-button uk-button-default  uk-button-small\" href=\"javascript:\">".Yii::t("default","Hist.")."</a>";
                    $action.="</div>";
                    ...
                    $feed_data['aaData'][]=array(
                        ...
                        "<small>Total:</small> ".prettyFormat($val['sub_total'],$merchant_id).
                        "<br><small>Tax:</small> ".prettyFormat($val['taxable_total'],$merchant_id).
                        "<br><small>W/Tax:</small> ".prettyFormat($val['total_w_tax'],$merchant_id),
        ...
        //ent@sobhon advanceFilterOption
	    ...
	    {	    		    	
            ...
            $status_history_list=FunctionsK::orderHistory($this->data['id']);
            $status_array=array();
            $final_data=array();
            if(!empty($status_history_list)){

                foreach ($status_history_list as $data){
                    $status_array[]=$data['status'];
                    //$status_array[$data['status']]=$data['status'];
                }
                $history_status=array_unique($status_array);
                $status_list=array_values($status_list);
                $diff1 = array_diff($history_status, $status_list);
                $diff2 = array_diff($status_list, $history_status);
                $final_array=array_merge($diff1, $diff2);
                foreach($final_array as $f_data){
	    	 ...
	    	 <?php if ( $res=Yii::app()->functions->getOrderInfo($this->data['id']) ):?>
	    	    ...
	    	    <?php echo CHtml::hiddenField('order_id',$this->data['id'])?>
		    	 ...
		    	 <div class="uk-form-row">
		    	  ...
		    	  <?php // echo CHtml::dropDownList('status',$res['status'],(array)$status_list,array('class'=>"uk-form-width-large"))?>
				  <?php echo CHtml::dropDownList('status',$res['status'],(array)$final_data,array('class'=>"uk-select"))?>
		    	 </div>
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		...
	    		foreach ($res as $val) {	    			
	    			...
	    			$feed_data['aaData'][]=array(
	    			  ...
	    			  prettyFormat($val['taxable_total'],$merchant_id),
	    			  prettyFormat($val['total_w_tax'],$merchant_id),
	    			   "<a data-id=\"".$val['order_id']."\" class=\"edit-order tag ".$val['status']."\" href=\"javascript:\">".t($val['status'])."</a>",
	    			  t($val['request_from']),
	    			  $date,
	    			  $action
	    		    );
        ...
        public function getMerchantIncomingOrder(){
            $DbExt= new DbExt();
			...
			(select color from {{order_recurring}} where id=a.recurring_id) as t_color,	    	
	    	(select concat(contact_phone) from {{client}} where client_id=a.client_id ) as contact_phone,	    	
	    ...
	    /**
         * All Merchant incoming order@sobhon
         * @getMerchantScheduledOrders for merchant
         *
         */
        public function getMerchantScheduledOrders(){
            $DbExt= new DbExt();
            $m_id=Yii::app()->functions->getMerchantID();
            $query_statement="SELECT a.*,
	    	(select concat(first_name,' ',last_name) from {{client}} where client_id=a.client_id ) as client_name,
		 ...
		 */
		 ...
		 /**
         * All Merchant incoming order@sobhon
         * @getMerchantPickupOrders for merchant
         *
         */
        public function getMerchantPickupOrders(){
            $DbExt= new DbExt();
            $m_id=Yii::app()->functions->getMerchantID();
            $query_statement="SELECT a.*,
	    	(select concat(first_name,' ',last_name) from {{client}} where client_id=a.client_id ) as client_name,
	    ...
	    {	    		    
			...
			//dump($stmt);
			//eperiment
			//count(recurring_id) as t_r_order,
...
$last_login=$val['last_login']=="0000-00-00 00:00:00"?"":date('M d,Y G:i:s',strtotime($val['last_login']));  */
        ...
        public function driverReviewRating(){
            $merchant_id=Yii::app()->functions->getMerchantID();
            $slug=$this->data['slug'];
            $stmt="SELECT a.*, concat(a.first_name,' ',a.last_name) as driver_name,b.rating_comment,b.rating
			FROM
			{{driver}} a
			...
			WHERE
			dropoff_merchant= ".FunctionsV3::q($merchant_id)."
			ORDER BY b.driver_id DESC";
            //dump($stmt);die();
            if ($res=$this->rst($stmt)){
                ...
                foreach ($res as $val) {
                    ...
                    $feed_data['aaData'][]=array(
                        ucwords($val['driver_name']).$action,
                        $val['rating_comment'].$comment,
                        /*$val['order_id'],*/
                        $val['rating'],
                        "$date<br/><span class=\"tag ".$val['status']."\">".t($val['status'])."</span>"
		...
		public function driverReviewRatingAdmin(){
            $merchant_id=Yii::app()->functions->getMerchantID();
            $slug=$this->data['slug'];
            $stmt="SELECT a.*, concat(a.first_name,' ',a.last_name) as driver_name,b.rating_comment,b.rating
			FROM
